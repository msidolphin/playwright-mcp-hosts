...
### Chromium `--host-resolver-rules` 失效

在 Chromium 146+ 上，即使正确传入 `--host-resolver-rules` 参数到浏览器进程和网络服务进程，DNS 覆盖依然不生效。原因可能与 macOS 上 Chromium 的 DNS 解析策略变更有关。本方案通过 CONNECT 代理在 TCP 层面直接重定向连接目标，完全绕过了浏览器的 DNS 解析机制。

### Chrome 网络服务进程共享

当系统 Chrome 正在运行时，Playwright 启动的 Chrome 实例可能与系统 Chrome 共享网络服务进程（`network.mojom.NetworkService`），导致 `--proxy-server` 等网络相关参数被忽略。使用 `msedge` 作为浏览器可避免此问题。
